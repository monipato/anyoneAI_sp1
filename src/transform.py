...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    #aggregations = (
    #    delivered
    #    .groupby("order_id")[["freight_value", "product_weight_g"]]
    #    .sum()
    #    .reset_index()
    #)
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_amount_per_date = (
        filtered_dates
        .groupby(filtered_dates["order_purchase_timestamp"].dt.date)
        .size()
    )
    ...
    # ensure that the date columns in both dataframes are in datetime format for the next step
    result_df["date"] = pd.to_datetime(result_df["date"],errors="coerce")
    holidays["date"] = pd.to_datetime(holidays["date"],errors="coerce")

    # Create the 'holiday' column by checking if the 'date' in result_df is in the 'date' column of holidays
